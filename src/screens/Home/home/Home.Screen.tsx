...
const HomeScreen = ({ navigation }: any) => {
    ...
    const renderPlaceCard = ({ item }: { item: AiRecommendationDTO }) => (
        ...
        >
            <Image
                source={{ uri: item.attraction.imageUrls?.[0] ?? "https://via.placeholder.com/300x140" }}
                style={styles.placeImage}
                resizeMode="cover"
            />

            {/* Phần hiển thị lý do AI với nền Gradient nhẹ */}
            <LinearGradient
            ...
            <View style={styles.placeInfo}>
                ...
                <Text style={styles.placeCategory}>{item.attraction.category}</Text>
                <View style={styles.placeRatingRow}>
                    <Ionicons name="star" size={12} color={COLORS.primary} />
                    <Text style={styles.placeRating}>{item.attraction.rating?.toFixed(1)}</Text>
                </View>
            </View>
        </TouchableOpacity>
    );

...
export function usePlaceDetail(navigation: any, placeId: number) {
  ...
  const loadPlaceDetail = async () => {
    setIsLoading(true);
    try {
      // getLocationById tự gọi kèm images
      const [placeRes, reviewRes, eventRes] = await Promise.allSettled([
        discoveryService.getLocationById(placeId),
        reviewService.getReviews(placeId, 0, 5),
        eventService.getEventsByLocation(placeId, 0, 5),
      ]);
  ...
  const handleToggleBookmark = async () => {
    if (!place) return;
    try {
      // toggleBookmark(locationId, currentlyBookmarked) → returns new state
  ...
  // Route params: locationId (placeId) và targetType = "place"
